...
public abstract class MongoDaoSupport<T,ID extends Serializable> implements IMongoDao<T,ID> {
     ...
     * @return
     */
    public boolean exists(Query query) { return   getMongoTemplate().exists(query, this.getEntityClass()); }
    ...
    private Update buildBaseUpdate(T t) {
        ...
        PropertyDescriptor[] fields = FiledUtils.getAllPropertyDescriptors(t.getClass());
        for(PropertyDescriptor descriptor :fields){
            try {
                update.set(descriptor.getName(), descriptor.getReadMethod().invoke(t,null));
            } catch (Exception e) {
            }
        }
        return update;
