...
public abstract class BaseMongoService<T,Mapper extends BaseMongoDao, ID extends Serializable> implements IMongoService<T, ID>{
    ...
    @Override
    public  T save(T t) { return (T)mongoDao.save(t); }
    ...
    @Override
    public T findById(ID id) { return (T) mongoDao.findById(id); }
    ...
    @Override
    public boolean exists(Query query){return mongoDao.exists(query);};
