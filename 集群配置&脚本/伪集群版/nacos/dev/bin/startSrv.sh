...
if [[ "$JAVA_MAJOR_VERSION" -ge "9" ]] ; then
  ...
  JAVA_OPT="${JAVA_OPT} -cp .:${BASE_NACOS_HOME}/plugins/cmdb/*.jar:${BASE_NACOS_HOME}/plugins/mysql/*.jar"

  JAVA_OPT="${JAVA_OPT} -Xlog:gc*:file=${BASE_DIR}/logs/nacos_gc.log:time,tags:filecount=10,filesize=102400"

else

  JAVA_OPT="${JAVA_OPT} -Djava.ext.dirs=${JAVA_HOME}/jre/lib/ext:${JAVA_HOME}/lib/ext:${BASE_NACOS_HOME}/plugins/cmdb:${BASE_NACOS_HOME}/plugins/mysql"
...
JAVA_OPT="${JAVA_OPT} -jar ${BASE_NACOS_HOME}/target/nacos-server.jar"
...
JAVA_OPT="${JAVA_OPT} --logging.config=${BASE_NACOS_HOME}/conf/nacos-logback.xml"
