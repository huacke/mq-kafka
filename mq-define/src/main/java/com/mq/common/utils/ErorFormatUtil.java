 ...
 * 简化生成异常栈链路
 */
public  class ErorFormatUtil {

    private static final Log log = LogFactory.getLog(ErorFormatUtil.class);
    ...
    private static void getFormatString(Throwable e, boolean filterPackage, StringBuilder exceptionResult,Integer level,int count) {
        ...
        for (StackTraceElement ste : stList) {
            if (filterPackage) {
                if (!ste.getClassName().contains(ErorFormatUtil.filterPackage)) {
                    continue;
                }
            }
